...
def train_normal():
    ...
    # 1. Carregar Pré-treino ImageNet (Fundamental para o Normal)
    ...
    # 2. Dados SEM Viés (bias_active=False é o segredo)
    ...
    # Wrapper simples para garantir que o treino recebe (img, label)
    # Nota: Aqui usamos bias_active=False, logo as imagens vêm LIMPAS.
    ...
    # Usamos 'trainval' para ter dados suficientes
    ...
    # 3. Treino (Fine-Tuning)
    criterion = nn.CrossEntropyLoss()
    # Usamos LR baixo porque o corpo já sabe ver (transfer learning)
    optimizer = optim.SGD([
        ...
        {'params': model.fc.parameters(), 'lr': 0.005} # LR maior só na cabeça nova
    ], momentum=0.9)
    
    model.train()
    epochs = 5 # 5 épocas chegam para alinhar a cabeça
